       ...
       <td width='40%'>
        ...
        <select name="kd_komponen" class="form-control" required>
            <option value="">Pilih Komponen</option>
                    <?php
                    include 'connect.php';
                    $sqlopt = "SELECT kd_komponen,nama_komponen FROM mst_komponen order by nama_komponen asc";
        ...
        <select name="kd_jeniskerusakan" class="form-control" required>
            <option value="">Pilih Jenis Kerusakan</option>
                    <?php
                    include 'connect.php';
                    $sqlopt = "SELECT kd_jeniskerusakan,jenis_kerusakan FROM mst_jeniskerusakan where kategori_kerusakan='Before Process' order by jenis_kerusakan asc";
                    $queryopt = mysql_query($sqlopt);
                    while($result=mysql_fetch_array($queryopt))
                    {
...
<td>
        ...
        <select name="kd_asalmasalah" class="form-control" required>
            <option value="">Pilih Lokasi Reject</option>
                    <?php
                    include 'connect.php';
                    $sqlopt = "SELECT kd_asalmasalah,nama_lokasiasalmasalah FROM mst_asal_masalah_komponen order by nama_lokasiasalmasalah asc";
                    $queryopt = mysql_query($sqlopt);
                    while($result=mysql_fetch_array($queryopt))
                    {
        ...
        <select name="kd_petugas" class="form-control" required>
            <option value="">Pilih Petugas</option>
                    <?php
                    include 'connect.php';
                    $sqlopt = "SELECT kd_petugas,nama_petugas FROM mst_petugas order by nama_petugas asc";
                    $queryopt = mysql_query($sqlopt);
                    while($result=mysql_fetch_array($queryopt))
                    {
